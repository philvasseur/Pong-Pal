...
def parseMessage(message):
	commandMap = {"help":commands.sendHelpOptions,"match":commands.handleMatchInput,"status":commands.sendRoomStatus, "history":commands.getMatchHistory,'stats':commands.getPlayerStats,'groups':commands.handleGroupsInput,'members':commands.handleMembersInput,'confirm':commands.confirmMatch, 'rankings': commands.displayRankings}
	text = message.text
	if len(text.split()) == 0:
